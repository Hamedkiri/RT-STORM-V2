...
import json, os, tempfile, random, re, collections
...
def last_epoch(dir_path: Union[str, Path],
    ...
    # Ex: "G_A_epoch*", "trainer_epoch*" etc.
    for p in dir_path.glob(f"{stem}_epoch*"):
        # filtre extension (autorise par nom complet si suffix ambigu)
...
def _remap_keys(sd: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
    ...
    NOTE (SPADE) :
      Les anciennes clés du style-encoder attentionnel (ex: 'style_enc.bot_attn',
      'style_enc.cross_blocks', etc.) n'ont **pas d'équivalent** dans SPADE/SEAN
      avec tokens multi-échelles. On les **ignore** volontairement.

    NOTE (SupHeads v2) :
      Les nouveaux SupHeads utilisent éventuellement 'mixers' au lieu de 'attentions'
      quand `token_mode="multi6"`. On NE transpose pas automatiquement les poids
      d’anciennes attentions → mixers (formes différentes) : ces clés seront
      simplement "unexpected" (et ignorées) si strict=False.
    """
    ...
    def _remap_one(k: str) -> Optional[Tuple[str, torch.Tensor]]:
        # filtre legacy style-encoder (attn) → ignoré
...
def _load_weights(model: nn.Module,
    ...
    """
    if not path.exists():
        print(f"[WARN] Poids manquant : {path}")
        return
    ...
    if path.suffix.lower() == ".safetensors":
        try:
            ...
            print(f"[WARN] safetensors n'est pas installé ({e}). Impossible de lire {path.name}.")
            return
    ...
    is_legacy_attn_style = any(
        any(k.startswith(pref) for pref in (
            "style_enc.bot_pre", "style_enc.bot_attn", "style_enc.bot_norm",
            "style_enc.to_mu", "style_enc.to_std", "style_enc.cross_blocks",
    ...
    if is_legacy_attn_style:
        print("[INFO] Checkpoint *legacy* détecté (AttnStyleEncoder). "
    ...
    try:
        ...
        if strict:
            print(f"[INFO] strict=True a échoué sur {path.name} — retry strict=False.\n      ({e})")
        missing, unexpected = model.load_state_dict(sd, strict=False)
        if missing:
            preview = ", ".join(missing[:6]) + ("…" if len(missing) > 6 else "")
            print(f"   → Clés manquantes ({len(missing)}) : {preview}")
        if unexpected:
            preview = ", ".join(unexpected[:6]) + ("…" if len(unexpected) > 6 else "")
            print(f"   → Clés inattendues ({len(unexpected)}) : {preview}")
...
# ────────────────────────────────────────────────────────────────
# 4) Sauvegarde “riche” SupHeads (meta + state_dict) — inchangé
# ────────────────────────────────────────────────────────────────
def save_supheads_rich(sup_heads: nn.Module, out_path: Path, *, safe_write: bool = True) -> None:
    ...
    => utilisé ensuite pour un load strict (mêmes hyperparams garantis).
    """
    ...
    print(f"✓ SupHeads (rich) sauvegardé → {out_path}")
...
def _atomic_save_safetensors(sd: Dict[str, torch.Tensor], fpath: Path, *, safe_write: bool = True) -> None:
    from torch.safetensors import save_file
    fpath = Path(fpath)
    fpath.parent.mkdir(parents=True, exist_ok=True)
    if not safe_write:
...
def _infer_jepa_meta(jepa_mod: nn.Module) -> Dict[str, Any]:
    ...
    Compatible avec la classe simple 'TokenJEPA' (MLP) et des variantes plus riches.
    """
    meta = {
        "D": None, "hidden": None, "heads": getattr(jepa_mod, "heads", None),
    ...
    # fouille sommaire du .net pour déduire D/hidden
    try:
...
def save_jepa_rich(jepa_mod: nn.Module, out_path: Path, *, safe_write: bool = True) -> None:
    ...
    print(f"✓ JEPA (rich) sauvegardé → {out_path}")
...
def _load_jepa_weights(jepa_mod: nn.Module, path: Path, device: torch.device, strict: bool = True) -> Dict[str, Any]:
    ...
    """
    if not path.exists():
        print(f"[WARN] JEPA manquant : {path}")
        return {}
    obj = torch.load(path, map_location=device)
    if isinstance(obj, dict) and "state_dict" in obj:
        sd = obj["state_dict"]; meta = obj.get("meta", {})
    else:
        sd = obj; meta = {}
    try:
        ...
        if strict:
            print(f"[INFO] JEPA strict=True a échoué sur {path.name} — retry strict=False.\n      ({e})")
        missing, unexpected = jepa_mod.load_state_dict(sd, strict=False)
        if missing:
            print(f"   → JEPA: clés manquantes ({len(missing)})")
        if unexpected:
            print(f"   → JEPA: clés inattendues ({len(unexpected)})")
    return meta
...
# ────────────────────────────────────────────────────────────────
# 7) Sauvegarde complète d’un checkpoint (+ Teachers + JEPA)
# ────────────────────────────────────────────────────────────────
def save_checkpoint(epoch: int, G_A, D_A, G_B, D_B,
                    ...
                    *,
                    ...
                    # ======== NOUVEAU : Semantic content (MoCo+JEPA) ========
    ...
    if use_safetensors:
        try:
            ...
            print(f"[WARN] safetensors indisponible ({e}) → fallback .pt")
    ...
    if sem_model is not None:
        try:
            ...
            print(f"[WARN] sauvegarde {sem_filename} impossible: {e}")
    ...
    if amp_scaler is not None:
        try: trainer["amp_scaler"] = amp_scaler.state_dict()
        except Exception: pass
    if sched_GA is not None:
        try: trainer["sched_GA"] = sched_GA.state_dict()
        except Exception: pass
    if sched_GB is not None:
        try: trainer["sched_GB"] = sched_GB.state_dict()
        except Exception: pass
    ...
    print(f"✓ checkpoint @epoch {epoch} sauvegardé → {out_dir}")




# ────────────────────────────────────────────────────────────────
# 8) Chargement checkpoints (+ Teachers + JEPA)
# ────────────────────────────────────────────────────────────────
def load_checkpoint(run_dir: Union[str, Path],
    ...
    def _find_weight_file(stem: str) -> Optional[Path]:
        for ext in prefer_exts:
            ...
            if p.exists():
                return p
        # fallback « n'importe quel ext connu »
    ...
    pa = _find_weight_file("G_A"); da = _find_weight_file("D_A")
    pb = _find_weight_file("G_B"); db = _find_weight_file("D_B")

    if pa is not None: _load_weights(G_A, pa, dev, strict=strict_GA)
    else:              print(f"[WARN] Fichier poids manquant: G_A_epoch{epoch}(.pt|.safetensors)")
    if da is not None: _load_weights(D_A, da, dev, strict=False)
    else:              print(f"[WARN] Fichier poids manquant: D_A_epoch{epoch}(.pt|.safetensors)")
    if pb is not None: _load_weights(G_B, pb, dev, strict=strict_GB)
    else:              print(f"[WARN] Fichier poids manquant: G_B_epoch{epoch}(.pt|.safetensors)")
    if db is not None: _load_weights(D_B, db, dev, strict=False)
    else:              print(f"[WARN] Fichier poids manquant: D_B_epoch{epoch}(.pt|.safetensors)")
    ...
    if sem_model is not None:
        ...
        if ps is not None:
            ...
            print(f"[INFO] Pas de poids trouvés pour {sem_filename} @epoch {epoch}")

    # --- Teachers EMA (si modules fournis) ---
    if T_A is not None:
        ta = _find_weight_file("T_A")
        if ta is not None: _load_weights(T_A, ta, dev, strict=strict_TA)
        else:              print(f"[INFO] Pas de poids teacher trouvés pour T_A @epoch {epoch}")
    if T_B is not None:
        tb = _find_weight_file("T_B")
        if tb is not None: _load_weights(T_B, tb, dev, strict=strict_TB)
        else:              print(f"[INFO] Pas de poids teacher trouvés pour T_B @epoch {epoch}")
    ...
    if tokJEPA_B is not None:
        ...
        if jp.exists():
            ...
            if alt is not None:
                ...
                print(f"[INFO] Pas de bundle JEPA B trouvé @epoch {epoch}")
    ...
    if opt_GA is not None and "opt_GA" in trainer:
        try: opt_GA.load_state_dict(trainer["opt_GA"])
        except Exception: pass
    if opt_DA is not None and "opt_DA" in trainer:
        try: opt_DA.load_state_dict(trainer["opt_DA"])
        except Exception: pass
    if opt_GB is not None and "opt_GB" in trainer:
        try: opt_GB.load_state_dict(trainer["opt_GB"])
        except Exception: pass
    if opt_DB is not None and "opt_DB" in trainer:
        try: opt_DB.load_state_dict(trainer["opt_DB"])
        except Exception: pass

    # SEM optimizer
    if opt_sem is not None and ("opt_SEM" in trainer) and (trainer.get("opt_SEM") is not None):
        try: opt_sem.load_state_dict(trainer["opt_SEM"])
        except Exception: pass

    # RNG CPU/CUDA
    if "rng_state" in trainer:
        try: torch.set_rng_state(trainer["rng_state"].cpu())
        except Exception: pass
    if "py_rng_state" in trainer:
        try: random.setstate(trainer["py_rng_state"])
        except Exception: pass
    if "np_rng_state" in trainer:
        try: np.random.set_state(trainer["np_rng_state"])
        except Exception: pass
    if torch.cuda.is_available() and ("cuda_rng_state_all" in trainer):
        try: torch.cuda.set_rng_state_all(trainer["cuda_rng_state_all"])
        except Exception: pass
...
def save_state_json(epoch: int,
                    ...
                    out_dir: Path) -> None:
    """
    Écrit un snapshot lisible (<out_dir>/train_state.json) incluant
    les réglages (λ par phase, boosts, NCE, FFT/SWD, EMA…) + la config SPADE
    + la section JEPA (si activée, étendue).
    """
    snapshot = {
        "schema_version": 7,
    ...
    print(f"✓ train_state.json mis à jour ({out_dir})")